...
public class BlockInteractTask implements QuestTask<BlockInteractTask.BlockInteractTaskResolved> {
    ...
    public static final QuestEntryKey<BlockInteractTask> ID = new QuestEntryKey<>(ResourceLocation.fromNamespaceAndPath(SimpleQuestsAPI.MODID, "block_interact"));
    public static final MapCodec<BlockInteractTask> CODEC = RecordCodecBuilder.mapCodec((instance) ->
            instance.group(Codec.BOOL.fieldOf("use").forGetter(d -> d.use),
                    ...
                    DescriptiveValue.withTranslation(BlockPredicate.CODEC).listOf()
                            .fieldOf("block_predicates").forGetter(d -> d.blockPredicates),
                    DescriptiveValue.withTranslation(ItemPredicate.CODEC).listOf()
    ...
    public record BlockInteractTaskResolved(DescriptiveValue<ItemPredicate> heldItem,
        ...
        public static final MapCodec<BlockInteractTaskResolved> CODEC = RecordCodecBuilder.mapCodec((instance) ->
                instance.group(DescriptiveValue.withTranslation(BlockPredicate.CODEC).optionalFieldOf("block").forGetter(d -> Optional.ofNullable(d.blockPredicate)),
                        Codec.BOOL.fieldOf("consume_item").forGetter(d -> d.consumeItem),

                        EntityPredicate.CODEC.optionalFieldOf("player_predicate").forGetter(d -> Optional.ofNullable(d.playerPredicate)),
                        DescriptiveValue.withTranslation(ItemPredicate.CODEC).optionalFieldOf("item").forGetter(d -> Optional.ofNullable(d.heldItem)),
        ...
        public boolean check(ServerPlayer player, BlockPos pos, boolean use) {
            ...
            if (use != this.use)
                return false;
            boolean b = (this.heldItem == null || this.heldItem.value().test(player.getMainHandItem())) &&
                    (this.blockPredicate == null || this.blockPredicate.value().matches(player.serverLevel(), pos));
