...
public abstract class QuestBase implements Comparable<QuestBase> {
    ...
    public List<MutableComponent> getDescription(ServerPlayer player, int idx) {
        QuestBase resolved = this.resolveToQuest(player, idx);
        if (resolved == null)
            return this.description.stream().map(s -> Component.translatable(s).withStyle(ChatFormatting.DARK_GREEN)).collect(Collectors.toList());
    ...
    public MutableComponent getFormattedWith(ServerPlayer player, int idx, Map<String, ResolvedQuestTask> resolvedTasks, ChatFormatting... subFormatting) {
        ...
        MutableComponent main = Component.literal("").append(this.getTask(player, idx).withStyle(ChatFormatting.LIGHT_PURPLE));
