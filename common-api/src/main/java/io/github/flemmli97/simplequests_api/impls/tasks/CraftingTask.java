...
public class CraftingTask implements QuestTask<CraftingTask.CraftingTaskResolved> {

    public static final QuestEntryKey<CraftingTask> ID = new QuestEntryKey<>(ResourceLocation.fromNamespaceAndPath(SimpleQuestsAPI.MODID, "crafting"));
    public static final MapCodec<CraftingTask> CODEC = RecordCodecBuilder.mapCodec((instance) ->
            instance.group(JsonCodecs.nonEmptyList(DescriptiveValue.withTranslation(ItemPredicate.CODEC), "Item predicates can't be empty").fieldOf("item_predicates").forGetter(d -> d.itemPredicates),
            ...
            ).apply(instance, (predicates, amount, desc, player) -> new CraftingTask(predicates, amount, desc, player.orElse(null))));
    ...
    @Override
    public QuestEntryKey<CraftingTask> getId() {
        return ID;
    }
    ...
    public record CraftingTaskResolved(DescriptiveValue<ItemPredicate> item,
        ...
        public static final MapCodec<CraftingTaskResolved> CODEC = RecordCodecBuilder.mapCodec((instance) ->
                instance.group(DescriptiveValue.withTranslation(ItemPredicate.CODEC).fieldOf("item").forGetter(d -> d.item),
                        ...
                        EntityPredicate.CODEC.optionalFieldOf("player_predicate").forGetter(d -> Optional.ofNullable(d.playerPredicate))
                ).apply(instance, (predicate, amount, player) -> new CraftingTaskResolved(predicate, amount, player.orElse(null))));
        ...
        public boolean check(ServerPlayer player, ItemStack stack) {
            return this.item.value().test(stack) && (this.playerPredicate == null || this.playerPredicate.matches(player, player));
        }
    }
}
