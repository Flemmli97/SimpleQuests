...
public abstract class BlockPredicateMixin implements PredicateTranslation {
    ...
    public List<MutableComponent> translation() {
        ...
        if (this.blocks != null)
            this.blocks.forEach(i -> formattedItems.add(new TranslatableComponent(i.getDescriptionId())));
        if (this.tag != null) {
            formattedItems.add(QuestUtils.tagsComponent(this.tag, Registry.BLOCK, Block::getName));
        }
