        ...
        for (int i = 0; i < 9; i++) {
            switch (i) {
                case 3 -> {
                    ...
                    yes.setHoverName(Component.translatable("simplequests.gui.yes").withStyle(Style.EMPTY.applyFormat(ChatFormatting.GREEN)));
                    inv.updateStack(i, yes);
                }
                case 5 -> {
                    ItemStack no = new ItemStack(Items.RED_WOOL);
                    no.setHoverName(Component.translatable("simplequests.gui.no").withStyle(Style.EMPTY.applyFormat(ChatFormatting.GREEN)));
