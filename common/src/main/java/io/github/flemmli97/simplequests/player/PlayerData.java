...
public class PlayerData {
    ...
    public boolean acceptQuest(QuestBase quest, int subQuestIndex) {
        ...
        this.player.sendMessage(new TranslatableComponent(ConfigHandler.lang.get("simplequests.accept"), resolved.getFormattedWith(this.player, prog.getQuestEntries())).withStyle(ChatFormatting.DARK_GREEN), Util.NIL_UUID);
        return true;
    }
    ...
    private void completeQuest(QuestProgress prog) {
        LootTable lootTable = this.player.getServer().getLootTables().get(prog.getLoot());
        ...
        if (!prog.getCommandToRun().isEmpty())
            this.player.getServer().getCommands().performCommand(this.player.createCommandSourceStack(), prog.getCommandToRun());
